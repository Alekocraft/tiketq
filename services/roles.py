...
GENERAL_CASES_ROLES = {"administrador", "gestor_ti", "analista_ti"}
BLOCK_GENERAL_CASES_ROLES = {"sugip", "ciberseguridad"}
TRIAGE_TARGETS = {
...
def is_admin(roles: Iterable[str] | None) -> bool:
    return 'administrador' in set(normalize_roles(roles))


def can_resolve(roles: Iterable[str] | None) -> bool:
    return bool(set(normalize_roles(roles)) & RESOLVER_ROLES)


def can_ingest(roles: Iterable[str] | None) -> bool:
    return bool(set(normalize_roles(roles)) & INGEST_ROLES)
